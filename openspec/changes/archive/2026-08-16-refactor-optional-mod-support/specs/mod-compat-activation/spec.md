...
title, handbook title/body). It SHALL NOT change the modid, the `.cvv` command, the `conquestvanillavom`
asset domain, the assembly/namespace, the config filename, or the handbook `pageCode`.

#### Scenario: Existing user updates the mod
- **WHEN** a user who already has `ModConfig/conquestvanillavom.json` updates to the re-scoped version
- **THEN** their existing config is loaded unchanged and their `.cvv` commands continue to work

### Requirement: Compatibility status is reported to the user
The `.cvv list` output SHALL include a compatibility-fixes section listing each optional fix with its
target mod, whether that mod is detected, and (for C#/Harmony fixes) whether it is enabled in config.

#### Scenario: User runs .cvv list
- **WHEN** a user runs `.cvv list`
