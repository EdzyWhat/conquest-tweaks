# terrain-slabs-connected-textures Specification

## Purpose
TBD - created by archiving change add-terrain-slabs-compat. Update Purpose after archive.
## Requirements
...
### Requirement: Slab fix is reported in .ctc list
The `.ctc list` compatibility-fixes section SHALL include the Terrain Slabs fix, showing its target
mod (`terrainslabs`), whether that mod is detected, and whether it is enabled in config.

#### Scenario: User runs .ctc list with Terrain Slabs installed
- **WHEN** a user runs `.ctc list` with `terrainslabs` present
