...
- **Group 2 (JSON compat patches) — `src/assets/conquesttweaks/patches/compatibility/<modid>/`:**
  ...
  meta-patch. Also ships `medievalarchitecture/texture-remap.json` (37 `addmerge` ops across 32 block
  files, generated by `build/generate-ma-compat-patches.py` from a local extraction — see
  `docs/HANDOFF-medievalarchitecture.md`): MA resolves archway/window/gate/roof/trapdoor textures via
  `AttributeRenderingLibrary`'s `{rock}`/`{wood}` templating, hardcoded to plain vanilla texture paths
  Conquest never overrides in place (rock, stonebrick, cobblestone, planks — Conquest reskins those via
  its own separate blocks/folders instead, since vanilla still actively uses the plain paths elsewhere
  — loose rocks, ore host rock, cobblestone paths, plank floors), so MA's archways render vanilla next
  to Conquest full blocks. Redirects each to Conquest's equivalent tile (verified across all 14 vanilla
  rock types + 12 wood types). MA's on-disk `assets/confession/` has no `domain` override, so its
  blocktypes are `confession:`-domain, not `game:` (only the *textures* it references live in `game:`).
  Three MA-referenced families need no fix at all — `agedbrick`, `wood/debarked`, `clay/daub/*`, and
  `wood/planks/generic` — Conquest already overrides those exact vanilla paths in place. `overlay/
  damagedstone`/`crackedcobblestone` have no Conquest equivalent and stay vanilla by necessity. Most ops
  target a `behaviors[N]` array index resolved per-file against MA 1.1.1 — fails safe (silent no-op) if
  a future MA release reorders behaviors; re-run the generator against a fresh extraction if so.
