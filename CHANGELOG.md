...
### Added
- **Medieval Architecture compatibility (vanilla-looking archways).** [Medieval Architecture](https://mods.vintagestory.at/show/mod/31540)
  resolves its archway/window/gate/roof/trapdoor stone and wood textures via
  `AttributeRenderingLibrary`'s `{rock}`/`{wood}` attribute templating, hardcoded to plain vanilla
  texture paths. Conquest never reskins several of those exact paths in place — it reskins rock,
  stonebrick, cobblestone, and planks via its own separate blocks/folders instead, since vanilla itself
  still actively uses the plain paths elsewhere (loose rocks, ore host rock, cobblestone paths, plank
  floors) — so an MA archway built next to a Conquest-textured wall rendered vanilla stone and visibly
  clashed. Fixed with 37 `addmerge` JSON-patch ops across 32 MA block files
  (`patches/compatibility/medievalarchitecture/texture-remap.json`), each gated `dependsOn`
  medievalarchitecture, redirecting the affected refs to Conquest's equivalent tile — verified across
  all 14 vanilla rock types and all 12 real wood types. Generated by a new dev tool,
  `build/generate-ma-compat-patches.py`, from a local extraction of the target mod; re-run it if MA
  restructures its texture paths or block-behavior arrays in a future release. A caught-early gotcha:
  MA's `assets/confession/` folder has no `domain` override in its `modinfo.json`, so its blocktypes
  register under the `confession:` domain, not `game:` (the game domain is only where the *textures*
  it references live) — the patches target `file: "confession:blocktypes/…"` accordingly. Three
  families MA references needed no fix at all: `agedbrick`, `wood/debarked`, `clay/daub/*`, and
  `wood/planks/generic` are already Conquest-overridden in place; `overlay/damagedstone`/
  `crackedcobblestone` have no Conquest equivalent and stay vanilla by necessity. See
  `docs/HANDOFF-medievalarchitecture.md`. Verified in-game (Conquest v1.0.7, MA v1.1.1, VS 1.22.6).
